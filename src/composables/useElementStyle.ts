 ...
 *
 * 合并顺序遵循 CSS 层叠规则：
 * 1. 先合并所有已启用 class 选择器的样式（按 classStyles 定义顺序，模拟 CSS 源码顺序，后者覆盖前者）
...
export function useElementStyle<T extends CanvasElementBase>(data: Ref<T>) {
  ...
  const style = computed(() => {
    ...
    /** 收集所有已启用 class 选择器的样式（按 classStyles 定义顺序，模拟 CSS 源码顺序） */
    const enabledClasses = new Set(data.value.classes);
    for (const [className, classCss] of Object.entries(canvasStore.convertClassStyles)) {
      if (enabledClasses.has(className)) {
