...
<template>
    ...
    <div ref="audioWrapEl" :data-canvas-id="data.id" class="canvas-media" @click.stop="handleSelect">
        ...
        <audio ref="audioEl" :id="data.id" :class="classes" :src="data.src" :controls="isPreview && data.controls" :autoplay="isPreview && data.autoplay" :muted="isPreview && data.autoplay" :loop="data.loop"></audio>
    </div>
</template>
