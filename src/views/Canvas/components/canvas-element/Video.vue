...
<template>
    ...
    <div ref="videoWrapEl" :data-canvas-id="data.id" class="canvas-media" @click.stop="handleSelect">
        ...
        <video ref="videoEl" :id="data.id" :class="classes" :src="data.src" :controls="data.controls" :autoplay="isPreview && data.autoplay" :muted="isPreview && data.muted" :loop="data.loop"></video>
    </div>
</template>
